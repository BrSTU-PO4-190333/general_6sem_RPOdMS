...
public class GalleryActivity extends Activity  {
    ...
    public void onResume(){
        ...
        try{
            File imagesDirectory=new File("/storage/emulated/0/Download/"); //вместо download свое
            images=searchImage(imagesDirectory);
            updatePhoto(Uri.parse(images.get(currentImage)));
        }catch(Exception e){
            nameView.setText("Ошибка: Папка '/storage/emulated/0/Download/' не найдена"); //вместо download свое
            Log.d("myLogs","Ошибка");
        }
    }
